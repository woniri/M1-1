...
![2026년 이동평균 및 변화율](../images/05_moving_average_change_rate.png)

**적용 기법**: `analysis.py`에서 2026년 여름(6/1~8/24) 일별 실측 평균기온에 **7일 이동평균**(`rolling(window=7).mean()`)을, 일별 최고기온에 **전일 대비 변화율(%)**(`pct_change()`)을 계산했습니다.

- **이동평균의 목적**: 하루하루의 날씨 노이즈(°C 단위로 들쭉날쭉한 실측값)를 눌러서, 그 밑에 깔린 "며칠 단위로 지속되는 흐름"(더워지는 국면인지 식는 국면인지)을 드러내기 위한 기법입니다. 위 그래프 위쪽 패널에서 연한 회색 실선(일별 실측)은 날마다 크게 튀지만, 굵은 초록 선(7일 이동평균)은 6월 하순 저점(약 21.5℃) → 8월 초 정점(약 32.1℃) → 이후 완만한 하강이라는 계절 전체의 흐름을 훨씬 선명하게 보여줍니다.
- **변화율의 목적**: 완만한 변화와 급격한 승온·냉각 시점을 구분하기 위한 기법입니다. 아래쪽 패널에서 막대가 클수록(빨강=상승, 파랑=하강) 전날과 비교해 그날 기온이 급변했다는 뜻입니다.
- **실제 적용 결과**: 이 기간 중 **가장 급격한 승온일은 2026-06-26(전일 대비 +25.0%, 30.0℃)**, **가장 급격한 냉각일은 2026-06-20(전일 대비 -32.4%, 23.0℃)**이었습니다. 두 사례 모두 6월(장마 직전) 구간에 몰려 있어, 8월의 폭염 집중 구간(8/1~8/11)은 오히려 "급변"보다는 "높은 수준이 여러 날 지속"된 것에 가깝다는 점을 대비해서 보여줍니다 — 이는 3장의 관찰(8월 폭염이 짧지만 강했다)을 이동평균·변화율이라는 별개의 기법으로 다시 한번 교차 검증한 결과이기도 합니다.
...
상세 수치·해석은 `docs/ANALYSIS_EXPLANATION.md` 7장, 시각화는 `images/06_stl_decomposition.png`·`images/07_forecast_backtest_comparison.png`, 원본 데이터는 `data/forecast_backtest_results.csv`·`data/september_forecast_model_comparison.csv`를 참고하세요.
