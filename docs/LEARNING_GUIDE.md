...
| ② 시계열 데이터 이해 | 트렌드/계절성, 이동평균·변화율·편차·감쇄 등 6가지 기법 활용 (`ANALYSIS_EXPLANATION.md`) |
...
| 시계열 분석 기법(예: 이동평균/변화율/구간 통계 등) 2가지 이상 결과가 리포트에 포함되어 있는가? | **예, 6가지.** 7일 이동평균, 전일 대비 변화율(%), 연도별·월별 집계(구간 통계), 평년 대비 편차(Anomaly), 연도별 누적합, 평년값+감쇄(Decay) 기반 예측 — `docs/REPORT.md` 3~6-1장, `docs/ANALYSIS_EXPLANATION.md` 5·5-1장 |
| 시각화가 필수 2개 이상 포함되어 있는가? | **예, 5개.**(막대/타임라인/히트맵/누적곡선/이동평균·변화율) `images/`, `docs/REPORT.md` 3~6-1장에 임베드 |
| 권장 시각화 1개를 추가해 총 3개 이상을 구성했는가? (권장) | **예.** 필수 기준(2개: 막대·타임라인)을 넘어 권장 추가 3개(히트맵·누적곡선·이동평균&변화율)로 총 5개, 권장 기준(3개 이상)도 충족 |
...
> ① 로딩: `fetch_kma_data.py::collect_observed()`가 기상청 API를 호출해 JSON 응답을 받는다 → ② 정제: 숫자 타입 변환, 결측치 1건을 선형보간으로 채움, 폭염/열대야 여부를 임계값으로 플래그화 → ③ 분석: `analysis.py`가 정제된 CSV를 읽어 `groupby`로 연도별·월별 집계, 평년 편차, 누적합을 계산하고 7일 이동평균·전일 대비 변화율도 함께 계산 → ④ 시각화: matplotlib/seaborn으로 5개 차트를 그려 `images/`에 저장. (`docs/ANALYSIS_EXPLANATION.md` 1장)
...
> 이 프로젝트는 6가지 기법을 각각 다른 목적으로 썼다. **이동평균(7일)**은 하루하루의 노이즈를 눌러 "지금 계절이 더워지는 중인지 식는 중인지" 완만한 흐름을 보려는 것이고, **변화율(전일 대비 %)**은 반대로 그 흐름이 하루 만에 크게 꺾이는 "급변 시점"을 잡아내려는 것이다(6월 하순에 몰려 있었고, 8월 폭염 구간은 급변이 아니라 높은 수준의 지속이었음을 확인). **편차(Anomaly)**는 "그 시기가 평소(평년)보다 얼마나 이례적이었는지"를 절대 기온보다 더 잘 보여주려는 것이고, **감쇄(Decay) 예측**은 "올여름의 더운 기세가 9월 들어 언제쯤 꺾일지" 시점을 추정하려는 것이다. (`docs/ANALYSIS_EXPLANATION.md` 4·5·5-1장)
